...
#define MAX_LOG_PAGES 2  // ONLY 2 pages (4 + 4 = 8 logs)
...
/* ===== LONG PRESS (0) DETECTION FOR LOG VIEW ===== */
unsigned long zeroPressStart = 0;
bool zeroPressed = false;
bool logViewActive = false;
const unsigned long ZERO_HOLD_TIME = 10000;  // 10 seconds
uint16_t logViewIndex = 0;                   // current log page
...
void displayTask(void *pvParameters) {
  ...
  while (1) {
    if (!menuActive && !modeSelected && !waitingForPassword && !waitingForID && !isRegisterMode && !isDeleteMode && !adminMenuActive && !waitingForNewPassword && !logViewActive) {
      ...
      searchFingerNonBlocking();  // Fingerprint runs continuously
    }
...
// void displayLogOnScreen(uint16_t page) {
...
//   int totalPages = (logIndex + LOGS_PER_PAGE - 1) / LOGS_PER_PAGE;

//   if (page >= totalPages) page = totalPages - 1;

//   // Newest logs first
//   int start = logIndex - (page * LOGS_PER_PAGE) - 1;
...
//     char buf[40];
//     sprintf(buf, "%lu %02d:%02d:%02d", log.employeeID, log.hour, log.minute, log.second);
...
void displayLogOnScreen(uint16_t page) {
  ...
  if (page >= MAX_LOG_PAGES) page = MAX_LOG_PAGES - 1;
  ...
  for (int i = start; i >= end; i--) {
    ...
    char buf[40];
    // sprintf(buf, "%lu %02d:%02d:%02d", log.employeeID, log.hour, log.minute, log.second);
    sprintf(buf, "%lu %02d/%02d/%lu  %02d:%02d:%02d", log.employeeID, log.day, log.month, log.year, log.hour, log.minute, log.second);
  ...
  // tft.setCursor(5, 200);
  // tft.print("B:Prev C:Next D:Exit");
}
...
void checkKeypad() {
  while (customKeypad.available()) {
    ...
    /* ========== SELECT For Display Logs ========== */
    // if (key == 'C' && logViewActive) {
    //   logViewIndex++;
    //   tft.fillRect(0, 50, 320, 190, TFT_WHITE);  // Clear display area
    //   displayLogOnScreen(logViewIndex);
    //   return;
    // }

    /* ========== SELECT For Display Logs ========== */
    if (key == 'C' && logViewActive) {
      if (logViewIndex < MAX_LOG_PAGES - 1) {
        logViewIndex++;
        displayLogOnScreen(logViewIndex);
      }
      return;
    ...
    /* ========== SELECT For Display Logs ========== */
    // if (key == 'B' && logViewActive) {
    //   if (logViewIndex > 0) logViewIndex--;
    //   tft.fillRect(0, 50, 320, 190, TFT_WHITE);  // Clear display area
    //   displayLogOnScreen(logViewIndex);
    //   return;
    // }
  ...
  /* ===== CHECK # HOLD TIME AND ADMIN PAGE ===== */
  if (zeroPressed && !logViewActive) {
    if (millis() - zeroPressStart >= ZERO_HOLD_TIME) {
      zeroPressed = false;
      logViewActive = true;
      logViewIndex = 0;

      Serial.println(">>> LOG VIEW MODE <<<");
      displayLogOnScreen(logViewIndex);
    }
  }
