...
void displayTask(void *pvParameters) {
  ...
  while (1) {
    if (!menuActive && !modeSelected && !waitingForPassword && !waitingForID && !isRegisterMode && !isDeleteMode) {
...
//         keyBuffer = "";
//         enteredNumber = 0;
//       } else if (key == 'A') {  //For Registration And Delete Menu
//         Serial.println("Registration Or Delete Button Pressed");
//         registerAndDeleteMenuFlag = true;
//       } else if (key == 'B') {  // UP Arrow
//         if (registerAndDeleteMenuFlag) {
//           Serial.println("UP Arrow Button Pressed");
//           registratinAndDeleteOptionDisplayOnlyRegisrationOnScreen();
//           // registerAndDeleteMenuFlag = false;
//           registerFlag = true;
//         }
//       } else if (key == 'C') {  // DOWN Arrow
//         if (registerAndDeleteMenuFlag) {
  ...
  while (customKeypad.available()) {
    ...
    if (key == 'D') {
      ...
      if (menuActive && !modeSelected) {
        ...
        Serial.println("Enter Password then press D");
        return;
      }
      ...
      if (waitingForPassword) {
        ...
        if (enteredNumber == ADMIN_PASSWORD) {
          ...
          waitingForID = true;
          Serial.println("Enter ID (1-127) then press D");
        } else {
