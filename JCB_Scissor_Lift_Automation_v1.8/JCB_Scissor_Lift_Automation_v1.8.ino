    ...
    if (key == 'D') {
      ...
      if (adminMenuActive && !adminModeSelected) {
        ...
        if (isPasswordMode) {
          ...
          displayMassegesOnScreen("Enter Previouse Admin Password");
          return;
...
bool searchFinger() {
  ...
  while ((millis() - startTime) < TIMEOUT) {
    ...
    delay(50);  // Small sensor polling delay
  }

  // ❌ Timeout happened
  if (p != FINGERPRINT_OK) {
    Serial.println("Finger not detected (timeout)");
    return false;
  }

  // ✅ Finger detected → continue normal flow
  p = finger.image2Tz(1);
  Serial.println("Convert image to template : " + String(p));
