...
/* ================= PASSWORD ================= */
#define ADMIN_PASS_ADDR 3950  // EEPROM memory location
...
void displayTask(void *pvParameters) {
  ...
  while (1) {
    if (!menuActive && !modeSelected && !waitingForPassword && !waitingForID && !isRegisterMode && !isDeleteMode && !adminMenuActive && !waitingForNewPassword) {
      ...
      // Fingerprint runs continuously (non-blocking)
...
void setup() {
  ...
  // NEW_ADMIN_PASSWORD = loadAdminPassword();

  // Serial.print("Current Admin Password: ");
  // Serial.println(NEW_ADMIN_PASSWORD);
    ...
    if (key == 'D') {
      ...
      if (adminMenuActive && !adminModeSelected) {
        ...
        if (isPasswordMode) {
          ...
          keyBuffer = "";
          Serial.println("Enter Previouse Admin Password");
          displayMassegesOnScreen("Enter Previouse Password");
          return;
      ...
      if (waitingForNewPassword) {
        ...
        displayMassegesOnScreen("New Password Updated!");
      ...
      if (waitingForPassword) {
        ...
        if (enteredNumber == NEW_ADMIN_PASSWORD) {
          ...
          displayMassegesOnScreen("Enter ID");
        } else {
          Serial.println("Wrong Password!");
          displayMassegesOnScreen("Wrong Password!");
          delay(1000);
...
bool searchFingerNonBlocking() {
  ...
  switch (fingerState) {
    ...
    case FP_WAIT_FINGER:
      ...
      p = finger.getImage();
      if (p == FINGERPRINT_OK) {
        fingerState = FP_IMAGE_CONVERT;
      }
      break;

    case FP_IMAGE_CONVERT:
      p = finger.image2Tz(1);
      if (p == FINGERPRINT_OK) {
...
// Write 1 byte
void eepromWriteByteForPassword(uint16_t addr, uint8_t data) {
...
// Read 1 byte
uint8_t eepromReadByteForPassword(uint16_t addr) {
  Wire.beginTransmission(EEPROM_ADDR);
  Wire.write(addr >> 8);
...
void saveAdminPassword(unsigned long pass) {
  for (int i = 0; i < 4; i++) {
    eepromWriteByteForPassword(ADMIN_PASS_ADDR + i, (pass >> (8 * i)) & 0xFF);
  }
  Serial.println("Admin password saved to EEPROM");
}
...
unsigned long loadAdminPassword() {
  ...
  for (int i = 0; i < 4; i++) {
    pass |= ((unsigned long)eepromReadByteForPassword(ADMIN_PASS_ADDR + i)) << (8 * i);
  }

  // EEPROM empty or corrupted
