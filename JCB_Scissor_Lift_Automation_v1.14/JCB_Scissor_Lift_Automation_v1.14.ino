...
unsigned long DEFAULT_ADMIN_PASSWORD = 1234;
...
unsigned long STAR_RESET_TIME = 15000;  // 15 seconds
...
void setup() {
  ...
  Serial.println("Admin Password: " + String(NEW_ADMIN_PASSWORD));
...
void checkKeypad() {
      ...
      if (waitingForID) {
        ...
        if (isRegisterMode) {
          ...
          if (fingerID == 0) {
            ...
            displayMassegesOnScreen("Memory Full");
            resetAllStates();
            return;
          }
  ...
  if (starPressed && !passwordResetDone) {
    if (millis() - starPressStart >= STAR_RESET_TIME) {
  ...
  void saveEmployeeID(uint8_t fingerID, uint16_t empID) {
    ...
    Serial.print(empID);
    Serial.print(" mapped to Finger ID ");
    Serial.println(fingerID);
  }
    ...
    switch (fingerState) {
      ...
      case FP_SEARCH:
        ...
        if (p == FINGERPRINT_OK) {
          ...
          displayMassegesOnScreen("FP Match");
          delay(1000);
          tft.fillRect(0, 50, 320, 190, TFT_WHITE);
          ...
          displayMassegesOnScreen("FP Not Match");
          delay(1000);
          tft.fillRect(0, 50, 320, 190, TFT_WHITE);
          fingerState = FP_IDLE;
          return false;
