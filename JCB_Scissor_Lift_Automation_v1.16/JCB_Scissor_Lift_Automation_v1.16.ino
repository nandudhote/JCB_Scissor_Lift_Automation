...
/* ================= I2C EEPROM ================= */
#define eepromAddr 0x57  // I2C address of external EEPROM on RTC module (AT24C32/64)
#define SDA 21
#define SCL 22
...
#define maxLogs 500               // Maximum number of logs (depends on EEPROM size)
#define logCountAddr 4090        // EEPROM address where total log count is stored (near end of EEPROM)
// #define TFT_W 240
// #define MSG_Y 110
// #define LINE_H 22
// #define MAX_LINES 3
...
uint16_t readLogCount() {
  return (eepromReadByte(logCountAddr) << 8) |  // Read high byte
         eepromReadByte(logCountAddr + 1);      // Read low byte
}
...
void writeLog(uint16_t index, LogEntry log) {
  uint16_t addr = index * logSize;  // Calculate EEPROM address
